...
function displayDate(k: string) {
  ...
  return `${MO[(m ?? 0) as unknown as number]?.slice(0, 3)} ${d}, ${y}`;
}
...
function tick() {
  try {
    ...
    if (!_buf) {
      ...
      for (let i = 0; i < len; i++) {
        const t = i / len;
        ch[i] = (Math.random() * 2 - 1) * (1 - t) ** 4 * 0.12;
      }
    }
...
/* ── component ── */
export function ChronoSelect({ value, ...props }: ChronoSelectProps) {
  const isControlled = value !== undefined;
  const selectedKey = value ?? null;
  const stateKey = isControlled ? (selectedKey ?? "empty") : "uncontrolled";
...
type ChronoSelectInnerProps = ChronoSelectProps & {
  initialSelected: string | null;
  isControlled: boolean;
};

function ChronoSelectInner({
  value,
  onChange,
  placeholder = "Pick a date" as string,
  sound = true as boolean,
  initialSelected,
  isControlled,
}: ChronoSelectInnerProps) {
  const [open, setOpen] = useState(false);
  const [uncontrolledSelected, setUncontrolledSelected] = useState<string | null>(initialSelected);
  const selected = isControlled ? (value ?? null) : uncontrolledSelected;
  ...
  const pick = (d: number) => {
    commitSelection(toKey(year, month, d));
  };
  ...
  const goToday = () => {
    ...
    setMonth(n.getMonth());
    commitSelection(toKey(n.getFullYear(), n.getMonth(), n.getDate()));
  };
                ...
                <AnimatePresence mode="wait" initial={false}>
                  ...
                  >
                    {weeks.map((week, wi) => (
                      ...
                      >
                        {week.map((d, ci) => {
                          if (d === null) return <div key={ci.toString()} />;
